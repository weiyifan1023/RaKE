...
class KnowledgeNeurons:
    def __init__(
        ...
        if self.model_type == "bert":
            ...
            # self.input_ff_attr = "attn.c_attn"
            # self.output_ff_attr = "attn.c_proj.weight"
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None):
        ...
        if self.model_type == "bert":
            mask_idx = torch.where(
                encoded_input["input_ids"][0] == self.tokenizer.mask_token_id  # find [MASK] token index
            )[0].item()
        else:
    ...
    def _generate(self, prompt, ground_truth):
        ...
        for i in range(n_sampling_steps):
            ...
            # print(target_idx)
            gt_prob = probs[:, target_idx]  # 获得target的prob,
            # print(gt_prob.shape)
    ...
    def intermediate_size(self):
        if self.model_type == "bert":
            ...
            return self.model.config.hidden_size * 4  # attn = 1 , FNN = 4
    ...
    def scaled_input(activations: torch.Tensor, steps: int = 20, device: str = "cpu"):
        ...
        """
        # activations: (B, ffn_size)
        tiled_activations = einops.repeat(activations, "b d -> (r b) d", r=steps)
        return (
            tiled_activations * torch.linspace(start=0, end=1, steps=steps).to(device)[:, None]  # ((r B), ffn_size)
        )
    ...
    def get_coarse_neurons(
        ...
        if adaptive_threshold is not None:  # (in the KN paper, they set adaptive_threshold=0.3)
            threshold = attribution_scores.max().item() * adaptive_threshold
        if threshold is not None:
            return torch.nonzero(attribution_scores > threshold).cpu().tolist()  # 满足给定阈值条件的张量中非零元素的索引列表。
        # 没设置 threshold
        s = attribution_scores.flatten().detach().cpu().numpy()
        return (
            torch.nonzero(attribution_scores > np.percentile(s, percentile)).cpu().tolist()
                )
    ...
    def get_refined_neurons(
        ...
        # prompts: List[str],
        # ground_truth: str,
        prompts_bag: List[List[str]],
        prompts_ground_truth: List[str],
        ...
        """
        assert isinstance(prompts_bag, list), "Must provide a list of different prompts to get refined neurons"
        assert 0.0 <= p < 1.0, "p should be a float between 0 and 1"
        coarse_neurons = []
        n_prompts = 0
        for ground_truth, prompts in tqdm(zip(prompts_ground_truth, prompts_bag), desc="Calculate different fact bags for specific relation"):
        ...
        # t = n_prompts * p  # 至少在n_prompts都出现1次的KN; 再*P
        t = total_coarse_neurons * p
        c = collections.Counter()
        for neurons in coarse_neurons:
            for kn in neurons:  # neurons 是每一个prompt对应的KNs,
                c[tuple(kn)] += 1
    ...
    def get_scores_for_layer(
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                ...
                # we do this by repeating the input across the batch dimension,
                # multiplying the first batch by 0, the second by 0.1, etc., until we reach 1
                scaled_weights = self.scaled_input(baseline_activations, steps=steps, device=self.device)
                scaled_weights.requires_grad_(True)  # (steps*B) x H
    ...
    def modify_activations(
        ...
        (gt_baseline_prob, argmax_baseline_prob, argmax_completion_str, _, ) = self._generate(prompt, ground_truth)  # 获得ground_truth的概率
        if not quiet:
            print(
        ...
        # 查看enhance or suppress后, 最大概率的输出
        # outputs = self.model(**encoded_input)
        # probs = F.softmax(outputs.logits[:, -1, :], dim=-1)  # mask_idx
        # # max_index = torch.argmax(probs)
        # # argmax_prob, argmax_id = probs.max(dim=-1)
        # # 获取前 10 个最大概率的索引
        # topk_values, topk_indices = probs.topk(k=5, dim=-1)
        # topk_indices = topk_indices.flatten().tolist()  # 展平为一维列表
        # argmax_str = self.tokenizer.decode(topk_indices)
        # print("New Output: {}, Probs: {}".format(argmax_str, topk_values))
        ...
        results_dict["after"] = {"gt_prob": new_gt_prob, "argmax_completion": new_argmax_completion_str, "argmax_prob": new_argmax_prob,}
        ...
        # topk_indices = topk_indices.flatten().tolist()  # 展平为一维列表
        # argmax_str = self.tokenizer.decode(topk_indices)
        # print("New Output: {}, Probs: {}".format(argmax_str, topk_values[0].tolist()))
    ...
    def modify_weights(
        ...
        Specifically, the weights of the second Linear layer in the ff
        are updated by adding or subtracting the value of the word embeddings for `target`.
        """
        ...
        (
            ...
            argmax_completion_str,
            argmax_tokens,
        ) = self._generate(prompt, target)  # 生成
        if not quiet:
            print(
                f"\nBefore modification - groundtruth probability: {gt_baseline_prob}\nArgmax completion: `{argmax_completion_str}`\nArgmax prob: {argmax_baseline_prob}"
            )
        ...
        # modify the weights
        # by subtracting the original prediction's word embedding and adding the target embedding
